...
import { getCategoriesData } from '@/modules/posts/posts.utility';
...
export class PostsService {
     ...
     async create(id: string, createPostDto: CreatePostDto) {
          try {
               const validate: boolean = await this.categoriesRepository.hasExistWithIds(
     ...
     async likes(userId: string, id: string) {
          try {
               const post: Post | null = await this.postsRepository.findById(id);
